...
Player opens game
  ...
  → AI analyzes FULL track → generates COMPLETE note chart (entire song)
  → Notes scroll toward player in perspective 3D view — from first beat to last
  → Player presses [1][2][3][4] to hold frets + [ENTER] to strum
  → Score + combo + accuracy tracked for full song duration
  → Song ends naturally at last note → results screen → leaderboard submission
  → Wallet connect prompt → Northcrypto acquisition funnel
```

### Full Song Chart Requirements
```typescript
interface NoteChart {
  songDurationMs: number;    // Full song length — chart must cover 100%
  totalNotes: number;        // All notes, every section, no skipping
  notes: Note[];             // Sorted by time, 0ms → songDurationMs
  sections: SongSection[];   // Intro, verse, chorus, bridge, outro — all charted
  
  // VALIDATION: last note timestamp must be within 5s of song end
  // VALIDATION: no section longer than 8s with zero notes (would feel empty)
  // VALIDATION: note density follows audio energy — quiet = sparse, loud = dense
}
```
...
class AudioEngine {
  ...
  // FULL SONG — buffer the entire track, never slice or truncate
  // AudioBuffer holds the complete decoded audio in memory
  // Typical 4-min song @ 44.1kHz stereo ≈ 80MB RAM — acceptable
  async loadFullSong(url: string): Promise<AudioBuffer> {
    const res = await fetch(url);
    const arrayBuffer = await res.arrayBuffer();
    return this.ctx.decodeAudioData(arrayBuffer); // complete song
  }
  
  // Beat detection runs on FULL buffer, not just intro
  detectBeats(buffer: AudioBuffer): BeatMap {
    // 1. Split ENTIRE song into onset strength signal
    // 2. Find ALL peaks across full duration
    // 3. Return complete timestamps + intensity for every beat
  }
  ...
  const response = await fetch("https://api.anthropic.com/v1/messages", {
    ...
    body: JSON.stringify({
      ...
      system: `You are a Guitar Hero chart generator. Given audio analysis data 
               (BPM, beat timestamps, spectral features), generate a COMPLETE 
               full-length note chart for the ENTIRE song — never truncate, 
               never generate only a preview or intro. Every beat section from 
               start to finish must have notes. Return ONLY valid JSON.`,
      messages: [{
        role: "user",
        content: `Generate a FULL SONG note chart (all ${audioFeatures.durationSeconds}s) for: ${JSON.stringify(audioFeatures)}`
      }]
    })
  });
...
❌ NEVER truncate, preview, or demo-clip a song — always play the FULL track
❌ NEVER generate a partial note chart — chart must cover 100% of song duration
❌ NEVER stop audio early — song plays from 0:00 to end unless player quits
❌ NEVER use time limits on gameplay — full song = full gameplay session
```
...
> **Tärkeää**: Demo-videossa näytetään OIKEA täysi biisi alusta loppuun — ei clip, ei preview, ei 30s näyte. Kamera voidaan leikata mutta peli pyörii koko biisin.

```
0:00 - Open game, no login required → "Instant access, zero friction"
0:15 - Paste YouTube URL (Thunderstruck - AC/DC, 4:52 full song)
0:30 - AI analyzes ENTIRE track → "Generating chart for full 4:52..."
0:50 - Game starts at 0:00, full song begins playing from the top
1:10 - Notes scroll, player hits PERFECT chain through verse + chorus
1:30 - Rock Meter fills, multiplier hits 4x mid-song → "Engaging full-length loop"
1:50 - Show song progress bar at ~1:24 / 4:52 — demonstrating full song in progress
2:05 - Connect wallet prompt (can appear during pause) → "Crypto integration"
2:20 - Leaderboard shows scores from full completed songs only
2:35 - "Create Northcrypto account to claim prizes" → "User acquisition funnel"
2:50 - Architecture: AI analyzes full audio buffer → complete chart → 60fps playback
```
...
| **Full song playback** | **100% of track, 0:00 → end** |
| **Note chart coverage** | **Every beat section charted, no gaps** |
